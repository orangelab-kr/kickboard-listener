...
function checkEnvironments() {
  ...
  const checkEnv = (key: string) => {
    if (typeof env[key] === 'string') return;
    throw Error(`${key} 환경변수 값이 올바르지 않습니다.`);
  };

  checkEnv('DATABASE_URL');

  checkEnv('KICKBOARD_SERVICE_HOSTNAME');
  checkEnv('KICKBOARD_SERVICE_USERNAME');
  checkEnv('KICKBOARD_SERVICE_PASSWORD');
  checkEnv('KICKBOARD_SERVICE_VHOST');
  checkEnv('KICKBOARD_SERVICE_QUEUE');
  checkEnv('KICKBOARD_SERVICE_MAX_QUEUE');

  checkEnv('HIKICK_OPENAPI_ISSUER');
  checkEnv('HIKICK_OPENAPI_LOCATION_URL');
  checkEnv('HIKICK_OPENAPI_LOCATION_KEY');
  checkEnv('HIKICK_OPENAPI_KICKBOARD_URL');
  checkEnv('HIKICK_OPENAPI_KICKBOARD_KEY');

  checkEnv('HIKICK_CORESERVICE_MONITORING_URL');
  checkEnv('HIKICK_CORESERVICE_MONITORING_KEY');
}
